...
interface ServerServicesViewProps {
...
export const ServerServicesView: React.FC<ServerServicesViewProps> = ({
  services,
  user,
  onSelectService,
  setActiveTab,
  showSearch = true,
  searchPlaceholder = 'Search Server/Credit Services...',
}) => {
  const [searchTerm, setSearchTerm] = useState('');

  const serverServices = services.filter(
    (s) => s.type === 'server' || s.serviceTypeGroup === 'Server/Credit' || s.category === 'remote_usb' || s.category === 'file_services' || s.serviceTypeGroup === '🛒 Server/Credit Service'
  );

  const displayList = serverServices.length > 0 ? serverServices : services;

  const query = searchTerm.toLowerCase().trim();
  const filteredServices = displayList.filter((service) => {
